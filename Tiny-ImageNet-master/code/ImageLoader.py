...
class Loader:
    ...
    def load_train_img(self, image_dir=None):
        ...
        images = np.ndarray(shape=(self.NUM_IMAGES, self.IMAGE_ARR_SIZE))
        labels = []
            ...
            if os.path.isdir(image_dir + imgdir[i] + '/images/'):
                for image in os.listdir(image_dir + imgdir[i] + '/images/'):
                    ...
                    if (image_data.shape==(self.IMAGE_SIZE, 
                                           self.IMAGE_SIZE, 
                                           self.NUM_CHANNELS)):
                        images[image_index, :] = np.reshape(image_data, [-1])
    ...
    def load_valid_img(self, testdir=None):
        ...
        images = np.ndarray(shape=(self.NUM_VAL_IMAGES, self.IMAGE_ARR_SIZE))
        image_index = 0
        labels = []
